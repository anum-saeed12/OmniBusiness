...
<section class="content">
    <div class="container-fluid">
        <div class="row mb-3">
            <div class="col-12">
                <a href="{{ route('sale.invoice.print.'.auth()->user()->user_role, $sales->invoice_id) }}" type="submit" class="btn btn-info toastrDefaultSuccess mr-2 btn-sm" target="btnActionIframe">
                    <i class="fa fa-print mr-1"></i> Print Invoice
                </a>
                <a href="{{ route('sale.invoice.'.auth()->user()->user_role, $sales->invoice_id) }}" type="submit" class="btn btn-info toastrDefaultSuccess btn-sm" target="btnActionIframe">
                            ...
                            <div class="row invoice-info">
                                ...
                                <div class="col-sm-4 invoice-col">
                                    ...
                                    <address>
                                        ...
                                        @if( ($sales->vendor != NULL))
                                        {{ ($sales->vendor->address_1 ) ?? "N/A" }}<br>
                                        {{ ($sales->vendor->address_2 ) ?? "N/A" }}<br>
                                        Phone: {{ ($sales->vendor->phone_num ) ?? "N/A" }}<br>
                                        Email: {{ ($sales->vendor->personal_email ) ?? "N/A" }}
                                        @endif

                                    </address>
                                </div>
                                <div class="col-sm-4 invoice-col">
                                    <label for="approved"> Sales Order Status</label> <br/>

                                    Status:
                                    @if( (\Carbon\Carbon::now()->toDateString() ) > (\Carbon\Carbon::createFromTimeStamp(strtotime($sales->due_date))->format('Y-m-d')))
                                    ...
                                    <div class="table-responsive table-sm">
                                        <table class="table">
                                            <tr>
                                                ...
                                                <td>{{ $currency }}{{number_format($sales->original_amount) }}</td>
                                            </tr>
                                            <tr>
                                                <th style="width:50%">After Discount Price:</th>
                                                <td>{{ $currency }}{{number_format($sales->total_amount) }}</td>
                                            </tr>
                                            <tr>
                                                <th>Tax ({{$gst}}%)</th>
                                            ...
                                            <tr>
                                                <th>Total:</th>
                                                <td>{{ $currency }}{{number_format($tax + ($sales->total_amount) )}}</td>
                                            </tr>
                                        </table>
                                    </div>
                                </div>
