...
<section class="content">
    <div class="container-fluid">
        <div class="row mb-3">
            <div class="col-md-6">
                <form method="POST" action="{{ route('department.store.'.auth()->user()->user_role) }}">
                    @csrf
                    <div class="row">
                        <div class="col-8">
                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover text-nowrap table-compact">
                            ...
                            <tr>
                                ...
                                <th class="pl-0">Total Employee's</th>
                            </tr>
                            </thead>
                            <tbody id="myTable">
                            @foreach($departments as $item)
                            <tr>
                                <td>{{ ucfirst($item['department']) }}</td>
                                <td >{{ $item['total'] }}</td>
                                <td class="text-right p-0">
                                    <a class="bg-danger list-btn"  href="{{ route('department.delete.'.auth()->user()->user_role, $item['id']) }}"><i class="fas fa-trash-alt" aria-hidden="false"></i></a>
                                </td>
                            </tr>
                            @endforeach
                            </tbody>
