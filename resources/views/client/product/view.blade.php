                ...
                <div class="card">
                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover text-nowrap table-compact">
                            ...
                            <tr>
                                <td><a href="{{ route("product.view.".auth()->user()->user_role, $product->id)}}"> {{ ucfirst($product->name ) }}</a></td>
                                <td><a href="{{ route("product.view.".auth()->user()->user_role, $product->id)}}"> {{ $product->unit }}</a></td>
                                <td><a href="{{ route("product.view.".auth()->user()->user_role, $product->id)}}"> {{ $product->unit_price }}</a></td>
                                <td><a href="{{ route("product.view.".auth()->user()->user_role, $product->id)}}"> {{ $product->in_stock }}</a></td>
                                <td><a href="{{ route("product.view.".auth()->user()->user_role, $product->id)}}"> {{ ucfirst($product->category->category->title) }}</a></td>
                                <td class="text-right p-0">
                                    <a class="bg-primary list-btn"  href="{{ route('product.edit.'.auth()->user()->user_role, $product->id)  }}"><i class="fas fa-tools" aria-hidden="false"></i></a>
                                    <a class="bg-danger list-btn"  href="{{  route('product.delete.'.auth()->user()->user_role, $product->id)  }}"><i class="fas fa-trash-alt" aria-hidden="false"></i></a>
                                </td>
                            </tr>
                            @endforeach
                            </tbody>
