...
@section('content')
    <section class="content">
        <div class="container-fluid">
            @include('manager.sale.components.filters')
            <nav>
                                ...
                                @if($sales->count() > 0)
                                    <div class="row mb-3 mt-3 ml-3">
                                        ...
                                        <div class="col-md-6 text-right pr-md-4">
                                            ...
                                            <a href="{{ route('sale.add.manager') }}" class="btn btn-success"><i class="fa fa-plus-circle mr-1"></i> Add New</a>
                                        </div>
                                    </div>
                                        ...
                                        <table class="table table-hover text-nowrap table-compact" id="table">
                                            ...
                                            @foreach($pos as $sale)
                                                <tr style="cursor:pointer" class="no-select" data-toggle="modal"
                                                    data-href="{{ route('sale.posview.manager', $sale->id) }}">
                                                    <td><a href="{{ route("sale.posview.manager", $sale->id) }}">{{ strtoupper( substr($sale->sale,6,6 )) }}</a></td>

                                                    <td><a href="{{ route("sale.posview.manager", $sale->id) }}">
                                ...
                                @if($sales->count() > 0)
                                    <div class="row mb-3 mt-3 ml-3">
                                        ...
                                        <div class="row offset-md-3 col-md-3">
                                            <form method="Get" action="{{ route('sale.list.manager') }}">
                                                <div class="input-group">
                                    ...
                                    <div class="card-body table-responsive p-0">
                                        <table class="table table-hover text-nowrap table-compact" id="table">
                                            ...
                                            @foreach($sales as $sale)
                                                <tr style="cursor:pointer" class="no-select" data-toggle="modal"
                                                    data-href="{{ route('sale.view.manager', $sale->id) }}">
                                                    <td><a href="{{ route("sale.view.manager", $sale->id) }}">{{ strtoupper( substr($sale->sale,6,6 )) }}</a></td>

                                                    <td><a href="{{ route("sale.view.manager", $sale->id) }}">
                                                        {{ \Carbon\Carbon::createFromTimeStamp(strtotime($sale->created_at))->format('d-M-Y')}}
                                                    </td>

                                                    <td>
                                                        @if($sale->quotation != Null )
                                                            <a href="{{ route("sale.view.manager", $sale->id) }}">
                                                    ...
                                                    <td><a href="{{ route("sale.view.manager", $sale->id) }}">{{ $currency }}{{ number_format($sale->total_amount) }}</a></td>

                                                    <td><a href="{{ route("sale.view.manager", $sale->id) }}">{{\Carbon\Carbon::createFromTimeStamp(strtotime($sale->due_date))->format('d-M-Y')}}</a> </td>
                                                    <td><a href="{{ route("sale.view.manager", $sale->id) }}">
