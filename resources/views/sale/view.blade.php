...
<section class="content">
    <div class="container-fluid">
        @include('sale.components.filters')
        <nav>
                                    ...
                                    <table class="table table-hover text-nowrap table-compact" id="table">
                                        ...
                                        @foreach($pos as $sale)
                                            <tr style="cursor:pointer" class="no-select" data-toggle="modal"
                                                data-href="{{ route('sale.posview.'.auth()->user()->user_role, $sale->id) }}">
                                                <td><a href="{{ route("sale.posview.".auth()->user()->user_role, $sale->id) }}">{{ strtoupper( substr($sale->sale,6,6 )) }}</a></td>

                                                <td><a href="{{ route("sale.posview.".auth()->user()->user_role, $sale->id) }}">
                                                ...
                                                <td><a href="{{ route("sale.posview.".auth()->user()->user_role, $sale->id) }}">{{ $currency }}{{ number_format($sale->total_amount) }}</a></td>
                                                @if(in_array(auth()->user()->user_role, ['client','manager']))
                                                <td class="text-right p-0">
                                                    <a class="bg-danger list-btn"  href="{{  route('sale.delete.'.auth()->user()->user_role, $sale->id)  }}"><i class="fas fa-trash-alt" aria-hidden="false"></i></a>
                                                </td>
                                                    @endif
                                            </tr>
                                        @endforeach
                            ...
                            @if($sales->count() > 0)
                                <div class="row mb-3 mt-3 ml-3">
                                    ...
                                    <div class="row offset-md-3 col-md-3">
                                        <form method="Get" action="{{ route('sale.list.'.auth()->user()->user_role) }}">
                                            <div class="input-group">
                                ...
                                <div class="card-body table-responsive p-0">
                                    <table class="table table-hover text-nowrap table-compact" id="table">
                                        ...
                                        @foreach($sales as $sale)
                                            <tr style="cursor:pointer" class="no-select" data-toggle="modal"
                                                data-href="{{ route('sale.view.'.auth()->user()->user_role, $sale->id) }}">
                                                <td><a href="{{ route("sale.view.".auth()->user()->user_role, $sale->id) }}">{{ strtoupper( substr($sale->sale,6,6 )) }}</a></td>

                                                <td><a href="{{ route("sale.view.".auth()->user()->user_role, $sale->id) }}">
                                                    {{ \Carbon\Carbon::createFromTimeStamp(strtotime($sale->created_at))->format('d-M-Y')}}
                                                </td>

                                                <td>
                                                    @if($sale->quotation != Null )
                                                        <a href="{{ route("sale.view.".auth()->user()->user_role, $sale->id) }}">
                                                ...
                                                <td><a href="{{ route("sale.view.".auth()->user()->user_role, $sale->id) }}">{{ $currency }}{{ number_format($sale->total_amount) }}</a></td>

                                                <td><a href="{{ route("sale.view.".auth()->user()->user_role, $sale->id) }}">{{\Carbon\Carbon::createFromTimeStamp(strtotime($sale->due_date))->format('d-M-Y')}}</a> </td>
                                                <td><a href="{{ route("sale.view.".auth()->user()->user_role, $sale->id) }}">
