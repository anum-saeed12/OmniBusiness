...
class SaleOrderController extends Controller
{
    ...
    private function pos(Request $request)
    {
        $client_id = Auth::user()->client_id;
        $pos = $this->all
            ?
            Sale::where('client_id', $client_id)
                ->where('due_date',null)
        ...
        if($request->filters) {
            ...
            if ($request->dates) {
                ...
                $stop = Carbon::createFromFormat('d/m/Y', $stop);
                # Add date filters to the query
                $pos->where("created_at", ">=", $start)->where("created_at", "<=", $stop);
            }
            if ($request->amount_min) {
                $request->validate([
                    'amount_min' => 'nullable|numeric'
                ...
                $amount_min = $request->amount_min;
                # Add min-amount filters to the query
                empty($amount_min) || $pos->where("total_amount", ">=", floatval($amount_min));
            }
            if ($request->amount_max) {
                $request->validate([
                    'amount_max' => 'nullable|numeric'
                ...
                $amount_max = $request->amount_max;
                # Add max-amount filters to the query
                empty($amount_max) || $pos->where("total_amount", "<=", floatval($amount_max));
            }
            if ($request->find) {
                $find = $request->find;
                # Add company-name-finder filters to the query
                empty($find) || $pos->where("buyer_name", "LIKE", "%{$find}%");
            }
            // Process code....
        }

        # Checks for filters
        $this->employee_id && $pos->where('employee_id', $this->employee_id);

        $poss = $pos->paginate($this->count);
        return $poss;
    }
    ...
    public function overview(Request $request)
    {
        ...
        $request->show_as && $request->validate(['show_as' => 'required|in:sale,date,employee,product,vendor']);

        $overview_basis = $request->show_as ? $request->show_as : 'date';

        $overview = (Object) [
            'year' => Sale::fetchSalesBy($overview_basis, 'year', $request->input('graph_type')),
           # 'month' => Sale::fetchSalesBy($overview_basis, 'month', $request->input('graph_type')),
            'day' => Sale::fetchSalesBy($overview_basis, 'day', $request->input('graph_type'))
        ];
        ...
        $data = [
            ...
            'base_url'         => env('APP_URL', 'http://omnibiz.local'),
            'user'             => Auth::user(),
            'year_sales'       => $overview->year['results'],
            'year_graph'       => $overview->year['graph'],
      /*    'month_sales'       => $overview->month['results'],
            'month_graph'       => $overview->month['graph'],*/
            'week_sales'       => /*$week_sales->get()*/[],
            #'today_sales'      => /*$today_sales->get()*/[],
            'today_sales'      => empty($overview->day['data']['main_label'])&&empty($overview->day['data']['total'])?[]:$overview->day['results'],
        ...
        return view('sale.overview', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = [
            ...
            'invoice_id' => $new_sale->id,
            'tax' => $quotation->gst
        ];
        ChartOfAccounting::record($data);

        return redirect(
            route('sale.list.client')
        )->with('success', 'Sale Order added successfully!');
    }
    ...
    public function invoice($id) {
        ...
        $date = "Invoice-". Carbon::now()->format('d-M-Y')  .".pdf";
        $pdf = PDF::loadView('sale.pdf-invoice', $data);
        return $pdf->download($date);
    }
