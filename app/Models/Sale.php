...
class Sale extends Model
{
    ...
    protected $fillable = ['client_id', 'employee_id', 'invoice_id', 'quotation_id', 'buyer_name',
        'tax_value', 'total_amount', 'sale', 'due_date'];

    protected static function booted()
    {
        static::updated(function ($sale) {
            # Add tax
            $tax = floatval($sale->tax_value)>0?$sale->tax_value/100:1;
            ...
            # Add to ledger
            # For sale
            $sale_ledger = new Ledger([
                'user_id' => Auth::id(),
                'client_id' => $sale->client_id,
                'account_type' => 'Sales',
                'transaction_id' => $transaction->id,
                'ledger_entry_type' => 'credit',
                'ledger_amount' => $sale->total_amount,
                'created_at' => $sale->created_at,
            ]);
            $sale_ledger->save();
            # Tax
            ...
            $cash_ledger = new Ledger([
                ...
                'ledger_amount' => $sale->total_amount + $tax_amount,
                'created_at' => $sale->created_at,
            ]);
            $cash_ledger->save();
        });
    ...
    {
        return $this->hasMany(SaleOrderline::class, 'sale_id', 'id')->with('product');
    }
    ...
    public static function fetchSalesBy($basis='date', $type=false)
    {
        $array = [
            'sale' => 'fetchSalesByDate',
            'date' => 'fetchSalesByDate',
            'employee' => 'fetchSalesByEmployee',
            'product' => 'fetchSalesByProduct',
            'vendor' => 'fetchSalesByVendor'
        ];
        $function = $array[$basis];
        return self::{$function}(Auth::user(), $type);
    }

    public static function fetchSalesByDate($user, $type=false)
    {
        $sale_orders = (new self())->getTable();
        $sale_order_line = (new SaleOrderline())->getTable();
        $results = [];
        $concatenated = [];
    ...
    public static function fetchSalesByEmployee($user, $type=false)
    {
        $sale_orders = (new self())->getTable();
        $sale_order_line = (new SaleOrderline())->getTable();
        ...
        if ($type == 'year') {
            $results = self::select(
                DB::raw("SUM({$sale_orders}.total_amount) as total"),
                DB::raw("{$employees}.id as employee_id"),
                DB::raw("CONCAT({$employees}.firstname,' ',{$employees}.lastname) as fullname"),
                DB::raw("COUNT(*) as counter"),
                DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%M, %Y') as creation_date")
            )->join($employees, "{$employees}.id", "=", "{$sale_orders}.employee_id")
            ->where("{$sale_orders}.created_at", ">=", Carbon::today()->firstOfYear())
            ->where("{$sale_orders}.client_id", $user->client_id)
            ->groupBy(
                "{$sale_orders}.employee_id",
                DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%M, %Y')")
            )
            ...
            foreach($employee_ids as $employee_id) {
                ...
                foreach ($period as $date) {
                    ...
                    $sale_data = self::select(
                        ...
                        ->where("{$sale_orders}.created_at", ">=", $month_start)
                        ->where("{$sale_orders}.created_at", "<=", $month_end)
                        ->where("{$sale_orders}.client_id", $user->client_id)
                        ->where("{$sale_orders}.employee_id", $employee_id)
                        ->groupBy(
                            DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%M, %Y')")
                        )
                        ->orderBy("{$sale_orders}.created_at", "DESC")
                    ...
                    $counter = isset($sale_data->counter)?$sale_data->counter:0;
                    $_counter[$employee_id] = isset($_counter[$employee_id])?$_counter[$employee_id].",{$counter}":$counter;
                    $total = isset($sale_data->total)?$sale_data->total:0;
        ...
        if ($type == 'month') {
            $results = self::select(
                ...
                DB::raw("CONCAT({$employees}.firstname,' ',{$employees}.lastname) as fullname"),
                DB::raw("COUNT(*) as counter"),
                DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%d %M, %Y') as creation_date")
            )->join($employees, "{$employees}.id", "=", "{$sale_orders}.employee_id")
                ->where("{$sale_orders}.created_at", ">=", Carbon::today()->firstOfMonth())
                ->where("{$sale_orders}.client_id", $user->client_id)
                ->groupBy(
                    "{$sale_orders}.employee_id",
                    DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%d %M, %Y')")
                )
                ->orderBy("{$sale_orders}.created_at", "DESC")
                ->get();
        ...
        if ($type == 'day') {
            $results = self::select(
                ...
                ->where("{$sale_orders}.created_at", ">=", Carbon::today())
                ->where("{$sale_orders}.client_id", $user->client_id)
                ->groupBy(
                    "{$sale_orders}.employee_id",
                    DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%M, %Y')")
    ...
    public static function fetchSalesByProduct($user, $type=false)
    {
        $sale_orders = (new self())->getTable();
        $sale_order_line = (new SaleOrderline())->getTable();
        ...
        if ($type == 'year') {
            $results = self::select(
                DB::raw("SUM({$sale_orders}.total_amount) as total"),
                DB::raw("{$sale_order_line}.product_id as product_id"),
                DB::raw("{$products}.name as product_name"),
                DB::raw("COUNT({$sale_orders}.id) as counter"),
                DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%M, %Y') as creation_date")
            )->rightJoin($sale_order_line, "{$sale_order_line}.sale_id", "=", "sale_orders.id")
            ->leftJoin($products, "{$products}.id", "=", "{$sale_order_line}.product_id")
                ->where("{$sale_orders}.created_at", ">=", Carbon::today()->firstOfYear())
                ->where("{$sale_orders}.client_id", $user->client_id)
                ->groupBy(
                ...
                ->orderBy("{$sale_orders}.created_at", 'ASC')
            ->get();
            ...
            foreach($product_ids as $product_id) {
                ...
                foreach ($period as $date) {
                    ...
                    $_results = self::select(
                        ...
                        ->leftJoin($products, "{$products}.id", "=", "{$sale_order_line}.product_id")
                        ->where("{$sale_orders}.created_at", ">=", $start)
                        ->where("{$sale_orders}.created_at", "<=", $end)
                        ->where("{$sale_orders}.client_id", $user->client_id)
            ...
            foreach($product_ids as $product_id) {
                ...
                foreach ($period as $date) {
                    ...
                    $_results = self::select(
                        ...
                        DB::raw("{$products}.name as product_name"),
                        DB::raw("COUNT({$sale_orders}.id) as counter"),
                        DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%d %M') as creation_date")
                    )->rightJoin($sale_order_line, "{$sale_order_line}.sale_id", "=", "sale_orders.id")
                        ->leftJoin($products, "{$products}.id", "=", "{$sale_order_line}.product_id")
                        ->where("{$sale_orders}.created_at", ">=", $start)
                        ->where("{$sale_orders}.created_at", "<=", $end)
                        ...
                        ->where("{$sale_order_line}.product_id", $product_id)
                        ->groupBy(
                            DB::raw("DATE_FORMAT({$sale_orders}.created_at,'%d %M')")
                        )
                        ->orderBy("{$sale_orders}.created_at", 'ASC')
                        ->first();
       ...
       # if ($type == 'week') {}
        ...
        if ($type == 'day') {
            $results = self::select(
                ...
                ->where("{$sale_orders}.created_at", ">=", Carbon::today())
                ->where("{$sale_orders}.client_id", $user->client_id)
                ->groupBy("{$sale_order_line}.product_id")
                ->orderBy("{$sale_orders}.created_at", 'ASC')
                ->get();
            ...
            foreach($product_ids as $product_id) {
                ...
                foreach ($period as $date) {
                    ...
                    $_results = self::select(
                        ...
                        DB::raw("{$products}.name as product_name"),
                        DB::raw("COUNT({$sale_orders}.id) as counter"),
                        DB::raw("{$sale_orders}.created_at as creation_date")
                    )->rightJoin($sale_order_line, "{$sale_order_line}.sale_id", "=", "sale_orders.id")
                        ->leftJoin($products, "{$products}.id", "=", "{$sale_order_line}.product_id")
                        ->where("{$sale_orders}.created_at", ">=", $start)
                        ->where("{$sale_orders}.created_at", "<=", $end)
    ...
    public static function fetchSalesByVendor($user, $type=false)
    {
        $sale_orders = (new self())->getTable();
        $sale_order_line = (new SaleOrderline())->getTable();
        ...
        if ($type == 'year') {
            $results = self::select(
                ...
                ->rightJoin($companies, "{$companies}.name", "=", "{$sale_orders}.buyer_name")
                ->where("{$sale_orders}.created_at", ">=", Carbon::today()->firstOfYear())
                ->where("{$sale_orders}.client_id", $user->client_id)
        ...
        if ($type == 'day') {
            $results = self::select(
                ...
                DB::raw("{$companies}.id as company_id"),
                DB::raw("COUNT({$sale_orders}.id) as counter"),
                DB::raw("{$sale_orders}.created_at,'%M, %Y' as creation_date")
            )
                ->rightJoin($companies, "{$companies}.name", "=", "{$sale_orders}.buyer_name")
                ->where("{$sale_orders}.created_at", ">=", Carbon::today())
                ->where("{$sale_orders}.client_id", $user->client_id)
    ...
    public static function generateSimpleGraph($target, $labels, $dataset, $graph_name='Monthly sales')
    {
        $graph_name || $graph_name = Uuid::uuid4()->getHex();
