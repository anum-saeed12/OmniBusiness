            ...
            <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane show active" id="nav-pos" role="tabpanel" aria-labelledby="nav-pos-tab"><div class="row">
                        <div class="col-md-12">
                            <div class="card card-info">
                                <form class="form-horizontal" action="{{ route('sale.store.manager') }}" method="post">
                                    @csrf
                ...
                <div class="tab-pane " id="nav-sale" role="tabpanel" aria-labelledby="nav-sale-tab"><div class="row">
                        <div class="col-md-12">
                            ...
                            @endif
                            <div class="card card-info">
                                <form class="form-horizontal" action="{{ route('sale.pos.manager') }}" method="post">
                                    @csrf
                                    <div class="card-body pb-0">
                                        <div class="row">
                                            <div class="col-sm-6">
                ...
                $.ajax({
                    ...
                    <div class="col-sm-4 invoice-col">
                                        ...
                                        <div class="col-6">
                                            ...
                                            <div class="table-responsive table-sm">
                                                <table class="table">
                                                    ...
                                                    <tr>
                                                        <th>Tax ({{$gst}}%)</th>
                                                        <td>{{ $currency  }}${number_format((quotation.gst/100).toPrecision(2) * quotation.total_amount.toPrecision(2))}</td>
                                                    </tr>
                                                    <tr>
                                                        <th>Shipping:</th>
                                                        <td>{{ $currency }}0.00</td>
                                                    ...
                                                    <tr>
                                                        <th>Total:</th>
                                                        <td>{{ $currency }}${number_format(quotation.total_amount + (quotation.gst/100).toPrecision(1) * quotation.total_amount.toPrecision(1)) }</td>
                                                    </tr>
                                                </table>
                                            </div>
                                        </div>
                        ...
                        $.each(quotation.items, function (index, json) {
                            $('#product_list').append(
                                $("<tr></tr>")
                                    .html(`<td>${json.quantity}</td><td>${json.product.name}</td><td>${json.product_id}</td><td>${number_format(json.discount)}</td><td>{{ $currency }}${number_format(json.unit_price)}</td><td>{{ $currency }}${number_format(json.total_price)}</td>`) // Using ES6
    ...
    <script>
        ...
        function search() {
            ...
            if (ele.val() !== product_search) {
                ...
                $.ajax({
                    url: "{{ env('APP_URL') }}/manager/sales/autocomplete/" + product_search,
                    dataType: "json",
                    beforeSend: function(){
